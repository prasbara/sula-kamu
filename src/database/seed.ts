...
export function seedDatabase(): void {
  ...
  // 3. Seed Admin Users (with SHA-256 / scrypt hashed passwords)
  ...
  if (adminCount.count === 0) {
    ...
    insertAdmin.run('admin-super-01', 'superadmin', hash, 'SULA Head Admin', 'SUPER_ADMIN');
    insertAdmin.run('admin-verifier-01', 'reviewer1', hash, 'Student Verifier 1', 'VERIFICATION_REVIEWER');
...
if (process.argv[1] && process.argv[1].endsWith('seed.ts')) {
  seedDatabase();
  console.log('Database successfully initialized and seeded with 32 Semarang institutions and system switches.');
}
